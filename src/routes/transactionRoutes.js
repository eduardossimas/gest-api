...
router.post('/transactions', 
    ...
    body('chart_of_account_id').notEmpty().withMessage('ID do plano de contas é obrigatório'),
    async (req, res) => {
        ...
        try {
            const query = 'INSERT INTO transactions (dueDate, paymentDate, type, description, value, user_id, bank_id, chart_of_account_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
            db.query(query, [dueDate, paymentDate, type, description, value, userID, bank_id, chart_of_account_id], (err, results) => {
                if (err) {
                ...
                db.query('SELECT currentBalance FROM banks WHERE id = ?', [bank_id], (err, bankResults) => {
                    if (err) {
                        console.error("Erro ao buscar saldo do banco:", err);
                        return res.status(500).json({ error: "Erro interno ao buscar saldo do banco" });
                    }
                    ...
                    db.query('UPDATE banks SET currentBalance = ? WHERE id = ?', [newBalance, bank_id], (err) => {
                        if (err) {
                            console.error("Erro ao atualizar saldo do banco:", err);
                            return res.status(500).json({ error: "Erro interno ao atualizar saldo do banco" });
                        }

                        res.status(201).json({ id: transactionID, message: "Transação criada com sucesso." });
                    });
                });
            });
        } catch (error) {
    ...
    try {
        ...
        db.query(query, [id, userID], (err, results) => {
            ...
            db.query('DELETE FROM transactions WHERE id = ? AND user_id = ?', [id, userID], (err) => {
                ...
                // Atualizar saldo do banco
                db.query('SELECT currentBalance FROM banks WHERE id = ?', [transaction.bank_id], (err, bankResults) => {
                    if (err) {
                        console.error("Erro ao buscar saldo do banco:", err);
                        return res.status(500).json({ error: "Erro interno ao buscar saldo do banco" });
                    }
                    ...
                    if (transaction.type === "Entrada") {
                        newBalance = parseFloat(currentBalance) - parseFloat(transaction.value);
                    ...
                    db.query('UPDATE banks SET currentBalance = ? WHERE id = ?', [newBalance, transaction.bank_id], (err) => {
                        if (err) {
                            console.error("Erro ao atualizar saldo do banco:", err);
                            return res.status(500).json({ error: "Erro interno ao atualizar saldo do banco" });
                        }
    ...
    async (req, res) => {
        ...
        try {
            ...
            db.query(query, [id, userID], (err, results) => {
                ...
                db.query('UPDATE transactions SET dueDate = ?, paymentDate = ?, type = ?, description = ?, value = ?, bank_id = ?, chart_of_account_id = ? WHERE id = ? AND user_id = ?', 
                [dueDate, paymentDate, type, description, value, bank_id, chart_of_account_id, id, userID], (err) => {
                    if (err) {
